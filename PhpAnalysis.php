...
class PhpAnalysis
{
    ...
    public function SetSource($source, $source_charset = 'utf-8', $target_charset = 'utf-8')
    {
        ...
        if ($source != '') {
            $rs = true;
            // $this->sourceString = mb_convert_encoding($source, 'UTF-8', 'UTF-8');
            ...
            if (preg_match("/^utf/", $source_charset)) {
                ...
                $this->sourceString = mb_convert_encoding(mb_convert_encoding($source, 'utf-8', 'big5'), UCS2, 'utf-8');
            } else {
                $rs = false;
            }
        } else {
    ...
    public function GetFinallyResult($spword = ' ', $word_meanings = false)
    {
        ...
        foreach ($this->finallyResult as $v) {
            ...
            $data = $v['w'];
            $w = $this->_out_string_encoding($data);
            $v['w'] = $data;
            if ($w != ' ') {
                if ($word_meanings) {
                    $rsstr .= $spword . $w . $m;
    ...
    {
        if (str_starts_with($this->targetCharSet, "utf")) {
            $rs = 1;
        } else if (str_starts_with($this->targetCharSet, "gb")) {
            $rs = 2;
        } else if (str_starts_with($this->targetCharSet, "big")) {
            $rs = 3;
        } else {
            $rs = 4;
        }
